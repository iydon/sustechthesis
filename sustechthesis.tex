% 仅提供必要的内容，使用方式按照自己的风格即可。
% 如果选项冲突建议在GitHub上联系作者。
% 如果在如何排版上有问题不建议联系，请认真阅读lshort-zh。
%
% 不使用宏包或者文类形式的原因：
%   1. 文类绝大部分功能都是继承，使用者修改不容易。
%   2. 宏包不易懂，同样在修改上存在障碍。
%   3. 宏包文类许多选项可以退化为命令，无需面向对象编程。
% 当然想抽成宏包或文类很简单，有兴趣可以玩一下。
%
% 控制序列若无特殊说明，提供均使用中文命令。
% 一方面防止冲突，另一方面方便理解。
% 推荐使用文类ctexart
% 如果字体加粗失败建议使用如下选项
% \documentclass[AutoFakeBold,AutoFakeSlant]{ctexart}
%
% https://github.com/Iydon
%


\makeatletter % 将@作为控制序列的一部分，可理解为私有变量
    % 使用的宏包
    \usepackage{ifxetex}
        % 建议使用XeLaTeX编译，其他编译方式建议注释此处。
        \RequireXeTeX
    ...
    \newcommand\中文标题页{%
        ...
        \begin{center}
            ...
            \end{tabular}
            \vskip 1.35cm
            \includegraphics[width=0.85\textwidth]{logos/SUSTech_Logo_zh.pdf}
            \vskip 0.5cm
            {\粗体\宋体\小初 本科生毕业设计（论文）}
            \vskip 3.5cm
            {\粗体\宋体\三号
    ...
    \newcommand\英文标题页{%
        ...
        \begin{center}
            ...
            \vskip 1.8cm
            \includegraphics[width=0.9\textwidth]{logos/SUSTech_Logo_en.pdf}
            \vskip 1.6cm
            ...
            {\粗体\三号
                \begin{tabular}{rr}
                    ...
                    Thesis Advisor: & \下划线[9cm]{\指导老师@英} \\[10pt]
                \end{tabular}}
            \vskip 2.1cm
            \三号 Date: \时间@英
        \end{center}
    ...
    \newcommand\中文诚信承诺书{
        ...
        \hfill\下划线[1.6cm]{}年\下划线[1cm]{}月\下划线[1cm]{}日
        \end{spacing}
        \clearpage\restoregeometry
    }
    % 英文
    ...
    \newcommand{\参考文献}{
        ...
        \renewcommand{\refname}{\hfill 参考文献\hfill}
        \sectionmark{参考文献}
    }
    % 附录
    \newcommand{\附录}{
